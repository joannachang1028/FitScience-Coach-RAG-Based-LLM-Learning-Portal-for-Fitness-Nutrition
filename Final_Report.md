...
```
User Query → Streamlit UI → RAG Pipeline → Vector Search → LLM Generation → Response + Citations

Detailed Flow:
1. User Input → Streamlit captures query
2. Query Embedding → HuggingFace model converts to vector
3. Vector Search → FAISS finds top-8 similar chunks
4. Context Augmentation → Retrieved chunks + query → prompt
5. LLM Generation → Llama 3.2 1B generates answer
6. Citation Processing → Extract and format sources
7. Response Display → Show answer + sources to user
```

### Key Technical Decisions
- **Local LLM**: Chose Llama 3.2 1B via Ollama for privacy and cost-effectiveness
...
- Llama 3.2 1B generated natural, educational responses
...
- Systematic evaluation revealed both strengths and improvement areas
...
*Report prepared for Assignment 3: Build a Personal Learning Portal*  
*Course: Application of NL(X) and LLM*  
